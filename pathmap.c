...
static const char *mappings[64][2];
static size_t mapping_count = 0;
static int g_debug = 0;
static int g_dry_run = 0;
static const char *excludes[64];
static size_t exclude_count = 0;
static unsigned char exclude_is_malloced[64];
static const char *g_current_syscall = NULL; /* debug context */

// Default exclusions when none provided via CLI or env
static const char *default_excludes[] = {
    "/etc/passwd",
    "/etc/group",
    "/etc/nsswitch.conf",
};
static const size_t default_exclude_count = sizeof default_excludes / sizeof default_excludes[0];

static void cleanup_excludes(void)
{
    for (size_t i = 0; i < exclude_count; i++) if (exclude_is_malloced[i]) free((void*)excludes[i]);
}

static void load_mappings(void)
{
	const char *env = getenv("PATH_MAPPING");
	mapping_count = 0;
	if (!env || !*env) {
		// Default mapping pairs when PATH_MAPPING/CLI not provided
		static const char *default_mappings[][2] = {
			{ "/tmp/path-mapping/tests/virtual", "/tmp/path-mapping/tests/real" },
		};
		size_t default_mapping_count = sizeof default_mappings / sizeof default_mappings[0];
		size_t limit = sizeof mappings / sizeof mappings[0];
		for (size_t i = 0; i < default_mapping_count && i < limit; i++) {
			mappings[mapping_count][0] = default_mappings[i][0];
			mappings[mapping_count][1] = default_mappings[i][1];
			mapping_count++;
		}
		return;
	}
	char **linear = NULL; char *buf = NULL; int pairs_len = 0;
	if (pm_parse_path_mapping_env(env, &linear, &pairs_len, &buf) != 0) return;
	size_t limit = sizeof mappings / sizeof mappings[0];
	for (int i = 0; i < pairs_len && mapping_count < limit; i++) {
		mappings[mapping_count][0] = strdup(linear[i * 2 + 0]);
		mappings[mapping_count][1] = strdup(linear[i * 2 + 1]);
		mapping_count++;
	}
	free(linear);
	free(buf);
...
{
    const char *env = getenv("PATH_MAPPING_EXCLUDE");
    exclude_count = 0;
    if (!env || !*env) {
        // Apply defaults
        size_t limit = sizeof excludes / sizeof excludes[0];
        for (size_t i = 0; i < default_exclude_count && i < limit; i++) {
            excludes[i] = default_excludes[i];
    ...
    const char *p = env;
    while (*p && exclude_count < (sizeof excludes / sizeof excludes[0])) {
        const char *start = p;
        while (*p && *p != ',') p++;
...
static int is_excluded_prefix(const char *abs_path)
{
    ...
    for (size_t i = 0; i < exclude_count; i++) {
        if (pm_path_prefix_matches(excludes[i], abs_path)) return 1;
    }
    return 0;
}
...
// This avoids making a nested remote mmap syscall.
...
static inline int regs_write(pid_t pid, const TraceRegs *r) { return g_dry_run ? 0 : ptrace(PTRACE_SETREGS, pid, NULL, (void *)r); }
...
static inline int regs_write(pid_t pid, const TraceRegs *r) { if (g_dry_run) return 0; struct iovec iov = { .iov_base = (void *)r, .iov_len = sizeof(*r) }; return ptrace(PTRACE_SETREGSET, pid, (void *)NT_PRSTATUS, &iov); }
...
/* removed old x86-specific get/set helpers; using arch-agnostic versions */
...
static void remap_arg_path_with_stack_fallback(pid_t pid, TraceRegs *regs, enum arg_reg sel, int dirfd)
{
	...
	if (is_excluded_prefix(buf)) { if (g_debug) fprintf(stderr, "[pathmap] exclude: '%s'\n", buf); return; }
	char out[MAX_PATH];
	const char *mapped = apply_mapping(buf, out, sizeof out);
	if (mapped == buf) return;
	if (g_dry_run) { if (g_debug) fprintf(stderr, "[pathmap] dry-run: '%s' -> '%s' (no write)\n", buf, mapped); return; }
	if (write_string_if_fits(pid, paddr, mapped, orig_len) == 0) {
		if (g_debug) fprintf(stderr, "[pathmap] in-place: '%s' -> '%s'\n", buf, mapped);
		return;
	}
	...
	if (write_string_on_stack(pid, regs, mapped, &new_addr) == 0) {
		...
		if (!g_dry_run) ptrace(PTRACE_SETREGS, pid, NULL, regs);
		if (g_debug) fprintf(stderr, "[pathmap] stack: '%s' -> '%s' @0x%lx\n", buf, mapped, new_addr);
	}
}
                                        ...
                                        enum arg_reg sel2, int dirfd2)
{
	...
	if (ex1) { if (g_debug) fprintf(stderr, "[pathmap] exclude: '%s'\n", b1); }
	if (ex2) { if (g_debug) fprintf(stderr, "[pathmap] exclude: '%s'\n", b2); }
	if (ex1 && ex2) return;
	...
	if (g_dry_run) { if (g_debug) fprintf(stderr, "[pathmap] dry-run pair: '%s'->'%s' | '%s'->'%s'\n", b1, r1, b2, r2); return; }
	int need1 = (r1 != b1);
	int need2 = (r2 != b2);
	int fit1 = 0, fit2 = 0;
	if (need1 && write_string_if_fits(pid, a1, r1, o1) == 0) { fit1 = 1; if (g_debug) fprintf(stderr, "[pathmap] in-place: '%s' -> '%s'\n", b1, r1); }
	if (need2 && write_string_if_fits(pid, a2, r2, o2) == 0) { fit2 = 1; if (g_debug) fprintf(stderr, "[pathmap] in-place: '%s' -> '%s'\n", b2, r2); }
	if ((need1 && !fit1) && (need2 && !fit2)) {
		...
		if (write_two_strings_on_stack(pid, regs, r1, &na1, r2, &na2) == 0) {
			...
			if (!g_dry_run) ptrace(PTRACE_SETREGS, pid, NULL, regs);
			if (g_debug) fprintf(stderr, "[pathmap] stack-pair: '%s' -> '%s' @0x%lx | '%s' -> '%s' @0x%lx\n", b1, r1, na1, b2, r2, na2);
		}
		return;
	}
	if (need1 && !fit1) {
		unsigned long na = 0; if (write_string_on_stack(pid, regs, r1, &na) == 0) { set_reg_by_sel(regs, sel1, na); if (!g_dry_run) ptrace(PTRACE_SETREGS, pid, NULL, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: '%s' -> '%s' @0x%lx\n", b1, r1, na); }
	}
	if (need2 && !fit2) {
		unsigned long na = 0; if (write_string_on_stack(pid, regs, r2, &na) == 0) { set_reg_by_sel(regs, sel2, na); if (!g_dry_run) ptrace(PTRACE_SETREGS, pid, NULL, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: '%s' -> '%s' @0x%lx\n", b2, r2, na); }
	}
}

static const char *apply_mapping(const char *in, char *out, size_t out_size)
{
	const char *res = pm_apply_mapping_pairs(in, mappings, (int)mapping_count, out, out_size);
	if (g_debug && res != in) fprintf(stderr, "[pathmap] map %s: '%s' -> '%s'\n", g_current_syscall ? g_current_syscall : "", in, res);
	return res;
}
...
static const char *apply_reverse_mapping(const char *in, char *out, size_t out_size)
{
	for (size_t i = 0; i < mapping_count; i++) {
		const char *pair[1][2] = { { mappings[i][1], mappings[i][0] } };
...
/* legacy single-path helper removed (all call sites now use stack-fallback path) */

/* legacy helper removed after stack-fallback pair handling was added */
...
static void on_sys_enter(pid_t pid, TraceRegs *regs)
{
...
#endif
	// mkdir/mkdirat
	if (sysno == SYS_mkdir) { g_current_syscall = "mkdir"; if (g_debug) fprintf(stderr, "[pathmap] handle mkdir\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
	if (sysno == SYS_mkdirat) { g_current_syscall = "mkdirat"; if (g_debug) fprintf(stderr, "[pathmap] handle mkdirat\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, (int)get_arg(regs,0)); return; }
	// chroot
	if (sysno == SYS_chroot) { g_current_syscall = "chroot"; if (g_debug) fprintf(stderr, "[pathmap] handle chroot\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
	...
	if (sysno == SYS_chmod) { g_current_syscall = "chmod"; if (g_debug) fprintf(stderr, "[pathmap] handle chmod\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
	if (sysno == SYS_lchown) { g_current_syscall = "lchown"; if (g_debug) fprintf(stderr, "[pathmap] handle lchown\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
#ifdef SYS_fchownat
	...
	if (sysno == SYS_readlink) {
		...
		if (g_debug) fprintf(stderr, "[pathmap] handle readlink\n");
		...
		if (read_string(pid, paddr, path, sizeof path) == 0) {
			...
			if (mapped != path) {
				...
				if (write_string_if_fits(pid, paddr, mapped, strlen(path)) != 0) {
					if (write_string_on_stack(pid, regs, mapped, &new_addr) == 0) { set_arg(regs,0,new_addr); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: '%s' -> '%s' @0x%lx\n", path, mapped, new_addr); }
				}
			}
		}
		return;
	...
	if (sysno == SYS_readlinkat) {
		...
		if (g_debug) fprintf(stderr, "[pathmap] handle readlinkat\n");
		...
		if (read_string(pid, paddr, path, sizeof path) == 0) {
			...
			if (mapped != path) {
				unsigned long new_addr = 0;
				if (write_string_if_fits(pid, paddr, mapped, strlen(path)) != 0) {
					if (write_string_on_stack(pid, regs, mapped, &new_addr) == 0) { set_arg(regs,1,new_addr); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: '%s' -> '%s' @0x%lx\n", path, mapped, new_addr); }
				}
			}
		}
		return;
...
#ifdef SYS_openat2
	if (sysno == SYS_openat2) { g_current_syscall = "openat2"; if (g_debug) fprintf(stderr, "[pathmap] handle openat2\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, (int)get_arg(regs,0)); return; }
#endif
#ifdef SYS_name_to_handle_at
	if (sysno == SYS_name_to_handle_at) { g_current_syscall = "name_to_handle_at"; if (g_debug) fprintf(stderr, "[pathmap] handle name_to_handle_at\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, (int)get_arg(regs,0)); return; }
#endif
#ifdef SYS_statfs
	if (sysno == SYS_statfs) { g_current_syscall = "statfs"; if (g_debug) fprintf(stderr, "[pathmap] handle statfs\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
#endif
#ifdef SYS_statfs64
	if (sysno == SYS_statfs64) { g_current_syscall = "statfs64"; if (g_debug) fprintf(stderr, "[pathmap] handle statfs64\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
#endif
#ifdef SYS_mount
	if (sysno == SYS_mount) {
		g_current_syscall = "mount";
		if (g_debug) fprintf(stderr, "[pathmap] handle mount\n");
...
#endif
#ifdef SYS_umount2
	if (sysno == SYS_umount2) { g_current_syscall = "umount2"; if (g_debug) fprintf(stderr, "[pathmap] handle umount2\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RDI, AT_FDCWD); return; }
#endif
#ifdef SYS_mount_setattr
	if (sysno == SYS_mount_setattr) { g_current_syscall = "mount_setattr"; if (g_debug) fprintf(stderr, "[pathmap] handle mount_setattr\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, (int)get_arg(regs,0)); return; }
#endif
#ifdef SYS_statmount
	if (sysno == SYS_statmount) { g_current_syscall = "statmount"; if (g_debug) fprintf(stderr, "[pathmap] handle statmount\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, (int)get_arg(regs,0)); return; }
#endif
#ifdef SYS_inotify_add_watch
	if (sysno == SYS_inotify_add_watch) { g_current_syscall = "inotify_add_watch"; if (g_debug) fprintf(stderr, "[pathmap] handle inotify_add_watch\n"); remap_arg_path_with_stack_fallback(pid, regs, ARG_RSI, AT_FDCWD); return; }
#endif
#ifdef SYS_fanotify_mark
	if (sysno == SYS_fanotify_mark) {
		g_current_syscall = "fanotify_mark";
		if (g_debug) fprintf(stderr, "[pathmap] handle fanotify_mark\n");
	...
	if (sysno == SYS_pivot_root) {
		...
		if (g_debug) fprintf(stderr, "[pathmap] handle pivot_root\n");
		remap_pair_paths_with_stack(pid, regs, ARG_RDI, AT_FDCWD, ARG_RSI, AT_FDCWD);
		return;
	}
#endif
			...
			if (need_link || need_tgt) {
				...
				if (need_link && need_tgt) {
					if (write_two_strings_on_stack(pid, regs, mapped_target, &na_tgt, mapped_link, &na_link) == 0) {
						...
						if (g_debug) fprintf(stderr, "[pathmap] stack-pair: target '%s' -> '%s' @0x%lx | link '%s' -> '%s' @0x%lx\n", target, mapped_target, na_tgt, linkpath, mapped_link, na_link);
					}
				} else if (need_link) {
					if (write_string_if_fits(pid, laddr, mapped_link, strlen(linkpath)) != 0) {
						if (write_string_on_stack(pid, regs, mapped_link, &na_link) == 0) { set_arg(regs,1,na_link); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: link '%s' -> '%s' @0x%lx\n", linkpath, mapped_link, na_link); }
					}
				} else if (need_tgt) {
					if (write_string_if_fits(pid, taddr, mapped_target, strlen(target)) != 0) {
						if (write_string_on_stack(pid, regs, mapped_target, &na_tgt) == 0) { set_arg(regs,0,na_tgt); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: target '%s' -> '%s' @0x%lx\n", target, mapped_target, na_tgt); }
					}
				}
			}
		}
		...
		if (read_string(pid, taddr, target, sizeof target) == 0 &&
			...
			if (need_link || need_tgt) {
				...
				if (need_link && need_tgt) {
					...
					if (write_string_if_fits(pid, laddr, mapped_link, strlen(linkpath)) != 0) {
						if (write_string_on_stack(pid, regs, mapped_link, &na_link) == 0) { set_arg(regs,2,na_link); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: link '%s' -> '%s' @0x%lx\n", linkpath, mapped_link, na_link); }
					}
				} else if (need_tgt) {
					if (write_string_if_fits(pid, taddr, mapped_target, strlen(target)) != 0) {
						if (write_string_on_stack(pid, regs, mapped_target, &na_tgt) == 0) { set_arg(regs,0,na_tgt); regs_write(pid, regs); if (g_debug) fprintf(stderr, "[pathmap] stack: target '%s' -> '%s' @0x%lx\n", target, mapped_target, na_tgt); }
...
int main(int argc, char **argv)
{
	...
	if (!g_debug) { const char *ed = getenv("PATHMAP_DEBUG"); if (ed && *ed && strcmp(ed, "0") != 0) g_debug = 1; }

	if (cli_mapping && *cli_mapping) {
		char **linear = NULL; char *buf = NULL; int pairs_len = 0;
		if (pm_parse_path_mapping_env(cli_mapping, &linear, &pairs_len, &buf) == 0) {
			size_t limit = sizeof mappings / sizeof mappings[0];
			mapping_count = 0;
			for (int i = 0; i < pairs_len && mapping_count < limit; i++) {
				mappings[mapping_count][0] = strdup(linear[i * 2 + 0]);
	...
	if (cli_exclude && *cli_exclude) {
		// Parse comma-separated prefixes into excludes[]
		exclude_count = 0;
		const char *p = cli_exclude;
		while (*p && exclude_count < (sizeof excludes / sizeof excludes[0])) {
			const char *start = p;
			while (*p && *p != ',') p++;
			size_t len = (size_t)(p - start);
	...
	// Ensure we free any malloc'ed exclude strings on exit
	atexit(cleanup_excludes);
